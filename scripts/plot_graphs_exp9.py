"""
Script to analyze the data collected in a rosbag during the experiment regarding the Passive Human Case, with simulated injury to the subscapularis tendon.
The same trajectories were executed with multiple participants, so it is possible to select the participant to analyze 
at the beginning of the main function. Note that the value of the axial rotation angle was kep fixed at AR=60 degrees.
...
def main():
    participant_number = 1      # select the participant to analyze (available=1)
    repetition = 1              # select the repetition to analyze (available=1, 2, 3; each participant performed 3 repetitions of the same trajectory)
    ...
    path_to_bag = os.path.join(path_to_repo, 'Personal_Results', 'bags', 'experiment_9')
    ...
    if participant_number == 1:
        if repetition == 1:
            bag_file_name = 'exp9_luka_6.bag'
        elif repetition == 2:
            bag_file_name = 'exp9_luka_7.bag'
        elif repetition == 3:
            bag_file_name = 'exp9_luka_8.bag'

    # load the strainmap dictionary used in the experiment
    file_strainmaps = os.path.join(path_to_repo, 'Musculoskeletal Models','Strain Maps','Passive','SuS_injury_1_5_num_Gauss_3.pkl')
    AR_value = np.deg2rad(60)   # we know that the axial rotation angle was kept fixed at 60 degrees during the experiment, so we can directly slice the strainmap at this value
    ...
    start = np.array([50, 80])
    goal_1 = np.array([110, 50])
    goal_2 = np.array([50, 80])
    fig = plt.figure()
    ax = fig.add_subplot()
    ...
    ax.legend()
    ax.set_title("EXP9: Trajectory on strainmap (with human)")
    ...
    duration = 5
    distance = np.deg2rad(np.linalg.norm(goal_1 - start) + np.linalg.norm(goal_2 - goal_1))
    ...
    fig = plt.figure()
    ax = fig.add_subplot()
    cb =ax.imshow(strainmap.T, origin='lower', cmap='hot', extent=[-20, 160, 0, 144], vmin=0.6, vmax=2.1)
    fig.colorbar(cb, ax=ax, label = 'Strain [%]')
    ...
    # ax.set_xlabel("Time [@ 10Hz]")
    # ax.legend()
    # fig.suptitle("EXP9: individual shoulder angles (with human)")
    ...
    # ax.set_ylabel('Y [m]')
    # ax.set_zlabel('Z [m]')
    # ax.set_title("EXP9: 3D EE trajectories (with human)")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP9: EE cartesian position")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP9: EE orientation")
