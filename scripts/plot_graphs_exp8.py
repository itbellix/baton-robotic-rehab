"""
Script to analyze the data collected in a rosbag during the experiment regarding the Passive Human Case, with simulated injury to the infraspinatus tendon.
The same trajectories were executed with multiple participants, so it is possible to select the participant to analyze 
at the beginning of the main function. Note that the value of the axial rotation angle was kep fixed at AR=-40 degrees.
...
def main():
    participant_number = 2      # select the participant to analyze (available=1, 2)
    ...
    path_to_bag = os.path.join(path_to_repo, 'Personal_Results', 'bags', 'experiment_8')

    # define the name of the bag file to analyze, based on the participant and repetition selected at the beginning of the function
    t_0_percentage = 45
    t_final_percentage = 93

    if participant_number == 1:
        if repetition == 1:
            bag_file_name = 'exp8_luka_1.bag'
        elif repetition == 2:
            bag_file_name = 'exp8_luka_2.bag'
        elif repetition == 3:
            bag_file_name = 'exp8_luka_3.bag'
    elif participant_number == 2:
        if repetition == 1:
            bag_file_name = 'exp8_stijn_1.bag'
            t_0_percentage = 35
            t_final_percentage = 99
        elif repetition == 2:
            bag_file_name = 'exp8_stijn_2.bag'
            t_0_percentage = 28
            t_final_percentage = 99
        elif repetition == 3:
            bag_file_name = 'exp8_stijn_3.bag'
            t_0_percentage = 30
            t_final_percentage = 99

    # load the strainmap dictionary used in the experiment
    file_strainmaps = os.path.join(path_to_repo, 'Musculoskeletal Models','Strain Maps','Passive','IS_injury_1_5_num_Gauss_3.pkl')
    AR_value = np.deg2rad(-40)   # we know that the axial rotation angle was kept fixed at -40 degrees during the experiment, so we can directly slice the strainmap at this value
    ...
    start = np.array([60, 100])
    goal_1 = np.array([90, 100])
    goal_2 = np.array([70, 50])
    goal_3 = np.array([60, 100])
    fig = plt.figure()
    ax = fig.add_subplot()
    ...
    ax.legend()
    ax.set_title("EXP8: Trajectory on strainmap (with human)")
    ...
    fig = plt.figure()
    ax = fig.add_subplot()
    cb =ax.imshow(strainmap.T, origin='lower', cmap='hot', extent=[-20, 160, 0, 144], vmin=01.0, vmax=3.2)
    fig.colorbar(cb, ax=ax, label = 'Strain [%]')
    ...
    # ax.set_xlabel("Time [@ 10Hz]")
    # ax.legend()
    # fig.suptitle("EXP8: individual shoulder angles (with human)")
    ...
    # ax.set_ylabel('Y [m]')
    # ax.set_zlabel('Z [m]')
    # ax.set_title("EXP8: 3D EE trajectories (with human)")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP8: EE cartesian position")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP8: EE orientation")
