...
import os
import utilities_from_Simbody as utils_simbody
...
# ------- FINDING THE INERTIA OF THE HUMAN ARM ----------------------------------------------------------------------------------------
...
humerus_inertia_translated_inHumFrame = utils_simbody.shiftFromMassCenter(humerus_inertia_simbody, translation_hum_gh_inHumFrame, humerus_mass) # update inertias to be all expressed at the same point - center of GH frame (scapula_offset) - but still body frames
ulna_inertia_translated_inUlnaFrame = utils_simbody.shiftFromMassCenter(ulna_inertia_simbody, translation_ulna_gh_inUlnaFrame, ulna_mass)
radius_inertia_translated_inRadiusFrame = utils_simbody.shiftFromMassCenter(radius_inertia_simbody, translation_radius_gh_inRadiusFrame, radius_mass)
hand_inertia_translated_inHandFrame = utils_simbody.shiftFromMassCenter(hand_inertia_simbody, translation_hand_gh_inRadiusFrame, hand_mass)
...
humerus_inertia_translated_inGhFrame = utils_simbody.rotateInertia(humerus_inertia_translated_inHumFrame, rotation_humFrameToGhFrame)
ulna_inertia_translated_inGhFrame = utils_simbody.rotateInertia(ulna_inertia_translated_inUlnaFrame, rotation_ulnaFrameToGhFrame)
radius_inertia_translated_inGhFrame = utils_simbody.rotateInertia(radius_inertia_translated_inRadiusFrame, rotation_radiusFrameToGhFrame)
hand_inertia_translated_inGhFrame = utils_simbody.rotateInertia(hand_inertia_translated_inHandFrame, rotation_handFrameToGhFrame)

arm_inertia_inGhFrame = utils_simbody.sumInertias(hand_inertia_translated_inGhFrame, utils_simbody.sumInertias(humerus_inertia_translated_inGhFrame, utils_simbody.sumInertias(ulna_inertia_translated_inGhFrame, radius_inertia_translated_inGhFrame)))
arm_inertia_inGhFrame_np = utils_simbody.fromInertiaToNumpyArray(arm_inertia_inGhFrame)
...
torques_to_apply = np.vstack((np.zeros((1,3)), np.array([1, 1, 1]).reshape((1,3))))
...
for index in range(torques_to_apply.shape[0]):
    ...
    torques_inHumFrame = np.matmul(utils_simbody.fromMat33toNumpyArray(rotation_humFrameToGhFrame).transpose(), torques_inGhFrame)
...
# T = I * omega_dot, rotated according to the values of the coordinates
...
R_simbody = osim.simbody.Rotation(osim.Mat33(R[0, 0], R[0, 1], R[0, 2],  R[1, 0], R[1, 1], R[1, 2], R[2, 0], R[2, 1], R[2, 2]))

omega_dot_base = np.matmul(np.linalg.inv(arm_inertia_inGhFrame_np), np.array([1, 1, 1]))
# omega_dot = np.matmul(R, omega_dot_base)
cosxy = osim.Vec2(np.cos(se_val), np.cos(ar_val))
sinxy = osim.Vec2(np.sin(se_val), np.sin(ar_val))
oocosy = 1/np.cos(ar_val)
w_PB = np.matmul(R, np.array([0, 0, 0]))
w_PB = osim.Vec3(w_PB[0], w_PB[1], w_PB[2])
qdot = R_simbody.multiplyByBodyXYZ_N_P(cosxy, sinxy, oocosy, w_PB)
b_PB = osim.Vec3(omega_dot_base[0], omega_dot_base[1], omega_dot_base[2])
omega_dot = R_simbody.convertAngAccInParentToBodyXYZDotDot(cosxy, sinxy, oocosy, qdot, b_PB).to_numpy()

# se_dotdot = np.matmul(np.array([-0.993, -0.0793, 0.0879]).reshape((1,3)), omega_dot)
# pe_dotdot = np.array([0.0828, 0.0654, 0.9944]) * omega_dot
# ar_dotdot = np.array([-0.0846, 0.9947, -0.0584]) * omega_dot

print(delta_acc)
print(omega_dot.reshape((3,)))


## Compute the omega_dot with self_implemented functions
cosxy_myself = np.array([np.cos(se_val), np.cos(ar_val)])
sinxy_myself = np.array([np.sin(se_val), np.sin(ar_val)])
q_dot_myself = utils_simbody.multiplyByBodyXYZ_N_P(cosxy_myself, sinxy_myself, oocosy, w_PB.to_numpy())
b_PB_myself = omega_dot_base
omega_dot_myself = utils_simbody.convertAngAccInParentToBodyXYZDotDot(cosxy_myself, sinxy_myself, oocosy, qdot, b_PB_myself)

print(omega_dot_myself)
