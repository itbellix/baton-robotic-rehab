...
# by providing the algorithm with a symbolic external function generated with a modified version of OpenSimAD.
...
F = ca.external('F_FD', os.path.join('/home/itbellix/Desktop/GitHub/opensimAD_fork/examples/simple_pendulum_FD' + '.so'))
F_map = np.load(os.path.join('/home/itbellix/Desktop/GitHub/opensimAD_fork/examples/simple_pendulum_FD' + '_map.npy'), allow_pickle=True).item() 

# Objective term
L = (x[0] - x_goal[0])**2 + u**2
cost = ca.Function('cost', [x, u], [L])
...
for k in range(N):
    ...
    # Evaluate ODE right-hand-side, and cost, at all helper states
    ode = ca.vertcat(Xc[1::, :], F(ca.vertcat(Xc, ca.repmat(Uk, d).T)))  # external function returns only acceleration, we need to provide velocities as well
    quad = cost(Xc, Uk)
