"""
Script to analyze the data collected in a rosbag during the experiment regarding the Passive Human Case, with simulated injury to the supraspinatus tendon.
...
def main():
    participant_number = 1      # select the participant to analyze (available=1, 2)
    ...
    path_to_bag = os.path.join(path_to_repo, 'Personal_Results', 'bags', 'experiment_6')
    ...
    if participant_number == 1:
        if repetition == 1:
            bag_file_name = 'exp6_luka_1.bag'
            t_0_percentage = 47
            t_final_percentage = 93
        elif repetition == 2:
            bag_file_name = 'exp6_luka_2.bag'
            t_0_percentage = 47
            t_final_percentage = 93
        elif repetition == 3:
            bag_file_name = 'exp6_luka_3.bag'
            t_0_percentage = 49
            t_final_percentage = 95

    elif participant_number == 2:
        if repetition == 1:
            bag_file_name = 'exp6_stijn_1.bag'
        elif repetition == 2:
            bag_file_name = 'exp6_stijn_2.bag'
        elif repetition == 3:
            bag_file_name = 'exp6_stijn_3.bag'

    # load the strainmap dictionary used in the experiment
    file_strainmaps = os.path.join(path_to_repo, 'Musculoskeletal Models','Strain Maps','Passive','SS_injury_1_5_num_Gauss_3.pkl')
    ...
    start = np.array([50, 90])
    goal_1 = np.array([50, 60])
    goal_2 = np.array([100, 60])
    goal_3 = np.array([70, 80])
    fig = plt.figure()
    ax = fig.add_subplot()
    ...
    ax.legend()
    ax.set_title("EXP6: Trajectory on strainmap (with human)")
    ...
    fig = plt.figure()
    ax = fig.add_subplot()
    cb =ax.imshow(strainmap.T, origin='lower', cmap='hot', extent=[-20, 160, 0, 144], vmin=0.65, vmax=2.3)
    fig.colorbar(cb, ax=ax, label = 'Strain [%]')
    ...
    # ax.set_xlabel("Time [@ 10Hz]")
    # ax.legend()
    # fig.suptitle("EXP6: individual shoulder angles (with human)")
    ...
    # ax.set_ylabel('Y [m]')
    # ax.set_zlabel('Z [m]')
    # ax.set_title("EXP6: 3D EE trajectories (with human)")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP6: EE cartesian position")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP6: EE orientation")
