"""
Script to analyze the data collected in a rosbag during the experiment regarding the Passive Human Case, with simulated injury to the teres minor tendon.
The same trajectories were executed with multiple participants, so it is possible to select the participant to analyze 
at the beginning of the main function. Note that the value of the axial rotation angle was kep fixed at AR=-20 degrees.
The plots produced here are used in Fig.TODO of the paper.
"""
...
def main():
    participant_number = 1      # select the participant to analyze (available=1, 2, 3)
    repetition = 3              # select the repetition to analyze (available=1, 2, 3; each participant performed 3 repetitions of the same trajectory)
    ...
    path_to_bag = os.path.join(path_to_repo, 'Personal_Results', 'bags', 'experiment_7')

    # define time percentages to select the part of the experiment to analyze (can be adjusted for each experiment specifically)
    t_0_percentage = 45
    t_final_percentage = 93

    if participant_number == 1:
        if repetition == 1:
            bag_file_name = 'exp7_TM_luka_1.bag'
        elif repetition == 2:
            bag_file_name = 'exp7_TM_luka_2.bag'
        elif repetition == 3:
            bag_file_name = 'exp7_TM_luka_3.bag'
    elif participant_number == 2:
        if repetition == 1:
            bag_file_name = 'exp7_TM_stijn_2.bag'
        elif repetition == 2:
            bag_file_name = 'exp7_TM_stijn_3.bag'
        elif repetition == 3:
            bag_file_name = 'exp7_TM_stijn_4.bag'
    elif participant_number == 3:
        if repetition == 1:
            bag_file_name = 'exp7_TM_micah_1.bag'
        elif repetition == 2:
            bag_file_name = 'exp7_TM_micah_2.bag'
            t_0_percentage = 40
            t_final_percentage = 99 
        elif repetition == 3:
            bag_file_name = 'exp7_TM_micah_3.bag'

    # load the strainmap dictionary used in the experiment
    file_strainmaps = os.path.join(path_to_repo, 'Musculoskeletal Models','Strain Maps','Passive','TM_injury_1_5_num_Gauss_3.pkl')
    AR_value = np.deg2rad(-20)   # we know that the axial rotation angle was kept fixed at -20 degrees during the experiment, so we can directly slice the strainmap at this value
    ...
    try:
        ...
        num_optim = 0
        print("No optimization data found in the bag file. This might be due to a problem during the recording of the bag file, or to a problem during the saving of the optimization outputs in the bag file.")
    ...
    init_time = xyz_curr[int(xyz_curr.shape[0]/100*t_0_percentage), -1]         # identify initial timestep
    end_time = xyz_curr[int(xyz_curr.shape[0]/100*t_final_percentage), -1] - init_time # identify final timestep
    ...
    strainmap = generate_approximated_strainmap(file_strainmaps, AR_value)
    ...
    ax.legend()
    ax.set_title("EXP7: Trajectory on strainmap (with human)")
    ...
    fig = plt.figure()
    ax = fig.add_subplot()
    cb =ax.imshow(strainmap.T, origin='lower', cmap='hot', extent=[-20, 160, 0, 144], vmin=0.8, vmax=2.1)
    fig.colorbar(cb, ax=ax, label = 'Strain [%]')
    ax.scatter(start[0], start[1], label = 'start', edgecolors='black', s=50, zorder=1)
    ax.scatter(goal_1[0], goal_1[1], label = 'goal1', edgecolors='black', s=50, zorder=1)
    ax.scatter(goal_2[0], goal_2[1], label = 'goal2', edgecolors='black', s=50, zorder=1)
    ax.scatter(goal_3[0], goal_3[1], label = 'goal3', edgecolors='black', c='pink', s=50, zorder=1)
    ...
    # ax.set_xlabel("Time [@ 10Hz]")
    # ax.legend()
    # fig.suptitle("EXP7: individual shoulder angles (with human)")
    ...
    # ax.set_ylabel('Y [m]')
    # ax.set_zlabel('Z [m]')
    # ax.set_title("EXP7: 3D EE trajectories (with human)")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP7: EE cartesian position")
    ...
    # ax.set_xlabel('time [s]')
    # ax.legend()
    # fig.suptitle("EXP7: EE orientation")
